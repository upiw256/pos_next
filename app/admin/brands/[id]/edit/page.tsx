...
import { getBrand, updateBrand } from "@/lib/actions/brand";
...
export default async function EditBrandPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!brand) {
    return <div>Merk tidak ditemukan.</div>;
  }

  const updateBrandWithId = updateBrand.bind(null, resolvedParams.id);

  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/brands" className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-white">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Merk</h1>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 dark:bg-gray-800 p-6">
        <form action={updateBrandWithId} className="space-y-4">
          <div>
            <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Merk</label>
            <input 
              type="text" 
              name="name" 
              id="name" 
